...
function stub() {
	...
	fn.returns = (val: any) => {
		resolveValue = val;
		fn._syncReturn = true;
		const wrapper: any = (...args: any[]) => { calls.push(args); return val; };
...
function createMockActions(overrides: Record<string, any> = {}): any {
	return {
		...
		prefetchAheadElements: stub().returns(undefined),
		isSkipContentRecheckable: stub().returns(false),
		...overrides,
	};
}
...
describe('PlaylistTraverser', () => {
	describe('processPlaylist - media elements', () => {
		...
		it('should retry when playElement returns RETRY', async () => {
			...
			expect(engine.control.sleep.calledWith(100)).to.equal(true);
		});
		...
		it('should not pass priorityCoord when priorityObject has no priorityLevel', async () => {
			...
			const args = engine.actions.playElement.calls[0];
			expect(args[8]).to.equal(undefined);
		});
		...
		it('should mark isLast correctly for the last element', async () => {
			...
			expect(engine.actions.playElement.calls[0][7]).to.equal(false);
			// Second call: isLast = true
			expect(engine.actions.playElement.calls[1][7]).to.equal(true);
		});
	});
	...
	describe('processPlaylist - dynamic content', () => {
		...
		it('should sleep when dynamic content detected but sync is off', async () => {
			...
			expect(engine.control.sleep.calledWith(1000)).to.equal(true);
	...
	describe('processPlaylist - seq repeatCount (no wallclock)', () => {
		...
		it('should sleep defaultAwait when conditional expired on last seq element', async () => {
			...
			expect(engine.actions.playElement.calls.length).to.equal(0);
			expect(engine.control.sleep.calls.some((c: any[]) => c[0] === 200)).to.equal(true);
		});
	});
	...
	// ===== activeWindowEnd stamping (future-begin priority campaign hold) =====
	// A wallclock-windowed definite seq/par under a priorityClass must record its
	// window end on the class's priority object when it starts playing, so
	// handlePriorityWhenDone can hold the cross-priority unpause between
	// repeatCount passes while the window is still open.

	describe('processPlaylist - activeWindowEnd stamping', () => {
		beforeEach(() => {
			MockDate.set(new Date('2025-06-15T12:00:00'));
		});
		...
		function makeLevelOnePriority(): PriorityObject {
			return { priorityLevel: 1, maxPriorityLevel: 1 } as PriorityObject;
		}

		it('stamps window end for an open non-repeating wallclock definite seq', async () => {
			const engine = createMockEngine();
			const traverser = new PlaylistTraverser(engine);
			const priorityObject = makeLevelOnePriority();
			const playlist = {
				seq: {
					repeatCount: '1',
					begin: 'wallclock(2025-06-15T10:00:00)',
					end: 'wallclock(2025-06-15T18:00:00)',
					video: makeVideo(),
				},
			} as any;

			await traverser.processPlaylist(playlist, 0, 'seq-parent', 0, priorityObject);

			expect(priorityObject.activeWindowEnd).to.equal(new Date('2025-06-15T18:00:00').getTime());
		});

		it('stamps window end for an open wallclock definite par', async () => {
			const engine = createMockEngine({ config: { defaultRepeatCount: '1' } });
			const traverser = new PlaylistTraverser(engine);
			const priorityObject = makeLevelOnePriority();
			const playlist = {
			...
			await traverser.processPlaylist(playlist, 0, '', 0, priorityObject);

			expect(priorityObject.activeWindowEnd).to.equal(new Date('2025-06-15T18:00:00').getTime());
		});

		it('does not stamp for a permanently expired wallclock seq', async () => {
			const engine = createMockEngine();
			const traverser = new PlaylistTraverser(engine);
			const priorityObject = makeLevelOnePriority();
			const playlist = {
			...
			expect(priorityObject.activeWindowEnd).to.equal(undefined);
		});

		it('does not stamp when there is no priority context', async () => {
			const engine = createMockEngine();
			const traverser = new PlaylistTraverser(engine);
			const priorityObject = {} as PriorityObject;
			const playlist = {
				seq: {
					repeatCount: '1',
					begin: 'wallclock(2025-06-15T10:00:00)',
