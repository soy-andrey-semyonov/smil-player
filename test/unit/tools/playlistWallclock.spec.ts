...
describe('Playlist tools component parseSmilSchedule tests', () => {
	...
	it('falls back gracefully when +w weekday hint has a non-digit char [4F regression]', async () => {
		...
		expect(Number.isFinite(result.timeToStart), 'timeToStart must not be NaN').to.equal(true);
		expect(Number.isFinite(result.timeToEnd), 'timeToEnd must not be NaN').to.equal(true);
