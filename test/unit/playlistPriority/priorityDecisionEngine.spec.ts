...
describe('PriorityDecisionEngine', () => {
	...
	describe('isEndTimeExpired', () => {
		it('should return true when endTime is past timestamp', () => {
			expect(isEndTimeExpired(Date.now() - 1000)).to.equal(true);
		});

		it('should return false when endTime is future timestamp', () => {
			expect(isEndTimeExpired(Date.now() + 10000)).to.equal(false);
		});

		it('should return false when endTime is repeat count (at or below threshold)', () => {
			expect(isEndTimeExpired(5)).to.equal(false);
			expect(isEndTimeExpired(ENDTIME_REPEAT_THRESHOLD)).to.equal(false);
		});

		it('should return false when endTime is 0 (indefinite)', () => {
			expect(isEndTimeExpired(0)).to.equal(false);
		});
	});

	describe('isRepeatCountExpired', () => {
		it('should return true when timesPlayed >= endTime', () => {
			expect(isRepeatCountExpired(3, 3)).to.equal(true);
			expect(isRepeatCountExpired(5, 3)).to.equal(true);
		});

		it('should return false when timesPlayed < endTime', () => {
			expect(isRepeatCountExpired(2, 3)).to.equal(false);
		});

		it('should return false when endTime is 0 (indefinite)', () => {
			expect(isRepeatCountExpired(100, 0)).to.equal(false);
		});
	});
