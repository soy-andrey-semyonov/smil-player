...
async function fetchReports(page: Page, baseUrl: string): Promise<ReportEntry[]> {
	const resp = await page.request.get(`${baseUrl}/cbp-loc/admin/log-fetch`);
	return await resp.json();
}

async function fetchContentLog(page: Page, baseUrl: string): Promise<ContentLogEntry[]> {
	const resp = await page.request.get(`${baseUrl}/cbp-loc/admin/content-log`);
	return await resp.json();
}

async function resetServerState(page: Page, baseUrl: string) {
	await page.request.post(`${baseUrl}/cbp-loc/reset`);
	await page.request.post(`${baseUrl}/cbp-loc/admin/log-reset`);
	await page.request.post(`${baseUrl}/cbp-loc/admin/content-log-reset`);
}
...
test.describe('checkBeforePlay (location): report URL ↔ on-screen content alignment', () => {
	test.beforeEach(async ({ context, testServerBaseUrl }) => {
		const SMIL_URL = `${testServerBaseUrl}/checkBeforePlayLocationLogged.smil`;
		await context.addInitScript(`window.__SMIL_URL__ = '${SMIL_URL}';`);
	});

	test('alignment holds during normal swap (clean propagation)', async ({ page, testServerBaseUrl }) => {
		const TEST_SERVER = testServerBaseUrl;
		await resetServerState(page, TEST_SERVER);
		...
		const reports = await fetchReports(page, TEST_SERVER);
		const contentLog = await fetchContentLog(page, TEST_SERVER);
	...
	test('alignment holds during slow-download race (Phase-4 commit lands during play)', async ({ page, testServerBaseUrl }) => {
		const TEST_SERVER = testServerBaseUrl;
		await resetServerState(page, TEST_SERVER);
		await page.goto(EMULATOR_URL, { waitUntil: 'load', timeout: 30000 });
		await clearStorage(page);
		...
		// Confirm the delay actually took effect, and pin the authoritative slow
		// download. The content delay is one-shot (consumed by the first GET), but
		// under load the player can fire a SECOND image3_v2.png GET — a lookahead
		// re-detects the change while the first (delayed) download is still in
		// flight; that GET finds the delay already consumed, returns instantly,
		// and — finishing first — is logged first. Selecting the first-logged
		// entry then reads durationMs≈0 and spuriously fails. Select the GET the
		// delay actually applied to (≥4000ms); its completion time is the
		// authoritative "new bytes landed" point the checks below rely on.
		const v2Gets = contentLog.filter((e) => e.file === 'image3_v2.png');
		const slowGet = v2Gets.find((e) => e.durationMs >= 4000);
		expect(
			slowGet,
			`expected a delayed image3_v2.png GET (>=4000ms); got durations ${JSON.stringify(v2Gets.map((e) => e.durationMs))}`,
		).toBeDefined();
