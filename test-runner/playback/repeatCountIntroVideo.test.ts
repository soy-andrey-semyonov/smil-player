...
test.describe('repeatCountIntroVideo.smil test', () => {
	test('processes smil file correctly', async ({ page, context, smilUrls, monitor }) => {
		...
		monitor.watch(durationCandidates('repeatCountIntroVideo'));

		// This fixture's point is that an intro tag is PRESENT in the SMIL (cf.
		// repeatCountNoIntro); whether the intro video visibly mounts is a race
		// against prefetch and not assertable. The player must process the intro
		// tag and then play the content sequence either way.
