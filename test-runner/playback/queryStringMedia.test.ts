...
test.describe('queryStringMedia.smil test', () => {
    test('media URLs with query params resolve via location header strategy (204 + Location)', async ({ page, context, smilUrls, monitor }) => {
        await context.addInitScript((url: string) => {
            (window as any).__SMIL_URL__ = url;
        }, smilUrls.queryStringMedia);
        await page.goto(`/?duid=${DUID}`);
        const frame = page.frameLocator('iframe');
        monitor.watch(durationCandidates('queryStringMedia'));
