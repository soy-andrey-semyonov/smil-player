...
test.describe('zonesCypress.smil test', () => {
	test('processes smil file correctly', async ({ page, context, smilUrls, monitor }) => {
		...
		monitor.watch(durationCandidates('zonesCypress'));

		// Step 1: Loader during prefetch — may be skipped with a warm asset cache
		await waitForLoaderOrSkip(page);

		// Step 2: Zone elements appear (firstElement budget absorbs cold boot + prefetch)
		const widgetImg1 = frame.locator('img[src*="widget_ima"][src*=".png"]').first();
		await expect(widgetImg1).toBeVisible({ timeout: Timeouts.firstElement });
