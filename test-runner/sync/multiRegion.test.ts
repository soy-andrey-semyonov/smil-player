...
test.describe('sync · multi-region', () => {
	...
	test('two synced regions advance independently with per-region lockstep', async ({
		...
		recordSkew({
			test: testInfo.title,
			label: 'top: l1→l2',
			skewMs: topSkew.skewMs,
			offsets: topSkew.timestamps.map((t) => t - topSkew.minTs),
		});
		...
		recordSkew({
			test: testInfo.title,
			label: 'bottom: l2→l1',
			skewMs: bottomSkew.skewMs,
			offsets: bottomSkew.timestamps.map((t) => t - bottomSkew.minTs),
		});

		// Rendered-DOM agreement: sample all four element candidates across all
		// devices simultaneously and assert they agree. Reuses the same locators
		// defined above — no duplication.
		const candidates: ElementCandidate[] = [
			{ name: 'top-l1', locator: topL1 },
			{ name: 'top-l2', locator: topL2 },
			{ name: 'bottom-l1', locator: bottomL1 },
			{ name: 'bottom-l2', locator: bottomL2 },
		];
		await assertRenderedElementAgreement(devices, candidates, {
			samples: 12,
			intervalMs: 500,
			label: 'multiRegion steady agreement',
		});
	});
});
