...
test.describe('sync · wallclock-triggered priority transition [8ef7571]', () => {
	...
	test('3 devices stay on matching syncIndex across the P_high → P_low wallclock boundary', async ({
		...
		// Persistent syncIndex divergence (spread > 1) or >2 distinct (syncIndex, visible)
		// tuples is the 8ef7571 regression signature. maxDivergentSnapshots defaults to 0:
		// a stalled slave would fall multiple indices behind for the rest of the run.
		const snapshots = await assertSyncIndexLockstepOverWindow(devices, {
			candidates,
			samples: SAMPLES,
			sampleGapMs: SAMPLE_GAP_MS,
			maxSpread: SYNC_SPREAD_MAX,
			label: '[wallclock-priority]',
		});
