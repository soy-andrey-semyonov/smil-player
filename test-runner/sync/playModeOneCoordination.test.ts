...
// healthy steady state truly is zero resync events per transition. The benign
// tail is small and bounded: the worst transient ever observed was a one-off
// `[0,4,0]` — a single device emitting a few recovery log lines after a
// momentary network blip, with sync otherwise healthy (every other assertion
// in this test passed that round). The db8da71 regression, by contrast, is an
// *infinite* resync loop that floods continuously (≫10 lines/transition), so
// the benign transient and the regression are cleanly separable by an order of
// magnitude. The threshold is therefore 6 — comfortably above the observed
// benign tail of 4 so the test passes deterministically at `--workers=1`
// without leaning on `retries`, and far below the regression's flood.
//
...
const MAX_RESYNC_DELTA_PER_TRANSITION = 6;
...
test.describe('sync · playMode=one coordination [db8da71, 5a59d20, 9faf699]', () => {
	...
	test('3 devices advance through 2 playMode=one cycles with <500ms skew per transition', async ({
		...
		// ------------------------------------------------------------------
		// B6: independently verify the per-cycle child SELECTION, not just the
		// transition timing. The hardcoded video→landscape1→landscape2 waits
		// above prove synchronized transitions but bake in the expected order;
		// here we capture what each device ACTUALLY renders over the next few
		// cycles WITHOUT assuming any order, then assert:
		//   (a) every device renders the SAME child at the same instant — the
		//       genuinely new axis: it catches a device painting a stale frame
		//       while still ACKing on time, which the WS/skew/frame checks above
		//       miss (same property as the `assertRenderedElementAgreement`
		//       helper); and
		//   (b) the selection ROTATES, i.e. is not stuck/degenerate on one child
		//       (a liveness guard — for this 3-child fixture it does NOT by itself
		//       distinguish playMode="one" from a plain seq, which the cmd-playMode
		//       log catchers above do; it guards against a collapsed selection).
		//
		// Both axes are captured in ONE sampling loop rather than via two helper
		// passes: the loop both collapses device 0's rendered sequence (which the
		// helper does not expose) AND runs the helper's exact re-sample-confirm
		// agreement check inline via the shared `isRenderedDisagreement` primitive,
		// so a second ~25 s sampling window on this already-slow sync test is avoided.
		// ------------------------------------------------------------------
		const candidates: ElementCandidate[] = [
			{ name: 'video', locator: video },
			{ name: 'landscape1', locator: l1 },
			{ name: 'landscape2', locator: l2 },
		];
		const TARGET_CYCLES = 4;
		const refDeviceSeq: string[] = [];
		const persistentDisagreements: Array<Array<string | null>> = [];
		const captureStart = Date.now();
		while (Date.now() - captureStart < 60_000) {
			const visible = await Promise.all(devices.map((d) => getVisibleElement(d.page, candidates)));
			// (a) A cross-device disagreement that survives a 250ms re-sample is a
			// real desync; a single-instant one is a transition straddle (one
			// device a few hundred ms ahead at the sample instant) and is ignored.
			if (isRenderedDisagreement(visible)) {
				...
				if (isRenderedDisagreement(confirm)) {
					persistentDisagreements.push(confirm);
				}
			}
			// (b) Record device 0's settled child (ignore null / "a+b" straddles),
			// collapsing consecutive duplicates into a per-cycle rotation sequence.
			const current = visible[0];
			if (current && current.indexOf('+') === -1 && refDeviceSeq[refDeviceSeq.length - 1] !== current) {
				refDeviceSeq.push(current);
			}
			if (refDeviceSeq.length > TARGET_CYCLES) {
				break;
			}
			await new Promise((r) => setTimeout(r, 350));
		}

		// (a) Coordination: no device ever painted a different child than its peers.
		expect(
			persistentDisagreements.length,
			`devices rendered different playMode=one children (selection not coordinated): ${JSON.stringify(persistentDisagreements)}`,
		).toBe(0);

		// (b) Rotation: the selection cycled through all 3 distinct children. A
		// regression that re-picks one element collapses this below 3.
		expect(
			refDeviceSeq.length,
			`expected to observe ≥${TARGET_CYCLES} playMode=one cycles, saw ${JSON.stringify(refDeviceSeq)}`,
		).toBeGreaterThanOrEqual(TARGET_CYCLES);
		expect(
			new Set(refDeviceSeq).size,
			`playMode=one must rotate through all 3 distinct children, saw ${JSON.stringify(refDeviceSeq)}`,
		).toBe(3);
	});
});
