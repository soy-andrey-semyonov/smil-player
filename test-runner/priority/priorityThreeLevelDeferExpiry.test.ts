...
test.describe('priorityThreeLevelDeferExpiry.smil test', () => {
	test('expired middle priority skipped, lowest priority plays after highest ends', async ({ page, context, smilUrls, monitor }) => {
		...
		monitor.watch(durationCandidates('priorityThreeLevelDeferExpiry'));
