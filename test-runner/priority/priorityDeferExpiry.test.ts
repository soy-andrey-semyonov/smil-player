...
test.describe('priorityDeferExpiry.smil test', () => {
	test('deferred element never plays when its wallclock window expires', async ({ page, context, smilUrls, monitor }) => {
		...
		monitor.watch(durationCandidates('priorityDeferExpiry'));
