...
test.describe('priorityDeferInterrupt.smil test', () => {
	test('deferred element survives blocker replacement by higher priority', async ({ page, context, smilUrls, monitor }) => {
		...
		monitor.watch(durationCandidates('priorityDeferInterrupt'));
