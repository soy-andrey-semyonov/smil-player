...
test.describe('triggersCrossCancel.smil test', () => {
	test('trigger1 starts content, trigger2 cancels it', async ({ page, context, smilUrls, monitor }) => {
		await context.addInitScript((url: string) => {
			(window as any).__SMIL_URL__ = url;
		}, smilUrls.triggersCrossCancel);
		await page.goto(`/?duid=${DUID}`);
		const frame = page.frameLocator('iframe');
		monitor.watch(durationCandidates('triggersCrossCancel'));
	...
	test('trigger2 has no effect without prior trigger1 activation', async ({ page, context, smilUrls, monitor }) => {
		await context.addInitScript((url: string) => {
			(window as any).__SMIL_URL__ = url;
		}, smilUrls.triggersCrossCancel);
		await page.goto(`/?duid=${DUID}`);
