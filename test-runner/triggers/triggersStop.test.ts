...
test.describe('triggersStop.smil test', () => {
	test('duration trigger expires and returns to default content', async ({ page, context, smilUrls, monitor }) => {
		await context.addInitScript((url: string) => {
			(window as any).__SMIL_URL__ = url;
		}, smilUrls.triggersStop);
		await page.goto(`/?duid=${DUID}`);
		const frame = page.frameLocator('iframe');
		monitor.watch(durationCandidates('triggersStop'));
	...
	test('self-cancel toggle stops trigger early on re-click', async ({ page, context, smilUrls, monitor }) => {
		await context.addInitScript((url: string) => {
			(window as any).__SMIL_URL__ = url;
		}, smilUrls.triggersStop);
		await page.goto(`/?duid=${DUID}`);
	...
	test('keyboard trigger with repeatCount=1 plays once and returns', async ({ page, context, smilUrls, monitor }) => {
		await context.addInitScript((url: string) => {
			(window as any).__SMIL_URL__ = url;
		}, smilUrls.triggersStop);
		await page.goto(`/?duid=${DUID}`);
