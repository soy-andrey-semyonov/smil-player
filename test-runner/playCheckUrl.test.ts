...
/**
 * playCheckUrl playability gate. The gate HEADs a per-element URL right before each
...
test.describe('playCheckUrl playability gate', () => {
	test('standalone gate: skips while closed, re-checks per pass, recovers, fails open on 500, no PoP while gated', async ({
		...
		// and the gate keeps re-HEADing image2 every pass (no persistent skip state).
		...
		// Per-pass recovery HEADs: with 2 remaining 3s slots + a ~100ms gate-skip pass,
		// each ~6s cycle re-checks image2 at least once.
		const gatedHeads = await gateHeadCount(page, testServerBaseUrl, 'image2');
		console.log(`[gate closed] image2 gate HEADs over ${NOT_SEEN_WINDOW_MS}ms: ${gatedHeads}`);
	...
	test('both checks: update lookahead and gate run as independent HEAD channels', async ({
		context,
		page,
		testServerBaseUrl,
	}) => {
		...
		// Gate channel: image2's gate URL is inline-HEADed once per pass despite the
		...
		console.log(`[both checks] image2 gate HEADs: ${gateHeads}`);
	...
	test('missing skipPlaybackOnHttpStatus meta: gate inert, zero gate HEADs', async ({
		context,
		page,
		testServerBaseUrl,
	}) => {
		...
		// Zero gate HEADs — the short-circuit fires before any request.
		const resp = await page.request.get(`${testServerBaseUrl}/gate/head-log`);
		const gateLog: unknown[] = await resp.json();
		console.log(`[missing meta] gate HEADs: ${gateLog.length}`);
	...
	// element, gate-closed keeps re-HEADing per pass, and the region never blanks once
	...
	test('priority interplay: gated higher element plays when open, re-checks per pass when closed, region never blanks', async ({
		...
		// Close the gate: image1 must keep being re-HEADed every pass (no persistent skip
		...
		console.log(`[priority, gate closed] image1 gate HEADs over ${NOT_SEEN_WINDOW_MS}ms: ${closedHeads}`);
