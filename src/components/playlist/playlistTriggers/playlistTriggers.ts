...
export class PlaylistTriggers extends PlaylistCommon implements IPlaylistTriggers {
	...
	// fix).
	private readonly listenerScope: ListenerScope = new ListenerScope();
			...
			async (event: CustomEvent) => {
				...
				if (this.triggersEndless[triggerId]?.play) {
					if (triggerMedia.seq?.end === triggerId) {
						debug(
							'[trigger-widget] cancelling via end-condition toggle: trigger=%s, region=%s',
							triggerId,
							this.triggersEndless[triggerId]?.regionInfo?.regionName,
						);
	...
	private isRegionOrNestedActive = (regionInfo: RegionAttributes): boolean => {
		// "Reserved for upcoming display" (findFirstFreeRegion sets playing=true ~5s before
		// cmd-play arrives) must not count as active — only "actually painted" should gate
		// the autonomous loop. Identity (triggerValue / dynamicValue) is set by
		// setCurrentlyPlaying alongside the visible flip, so it is the reliable signal.
		const isActuallyDisplaying = (entry: any): boolean =>
			!!entry?.playing &&
			(!!entry[SMILTriggersEnum.triggerValue] || !!entry[SMILDynamicEnum.dynamicValue]);
		...
		if (isActuallyDisplaying(this.currentlyPlaying.fullScreenTrigger)) {
			return true;
		}

		if (regionInfo.hasOwnProperty('region')) {
			for (const region of regionInfo.region as RegionAttributes[]) {
				if (isActuallyDisplaying(this.currentlyPlaying[region.regionName])) {
					return true;
				}
			}
		}
		...
		for (const [, value] of Object.entries(this.dynamicPlaylist)) {
			if (
				value.play &&
				regionInfo.regionName === value.parentRegion &&
				value.regionInfo?.regionName &&
				this.currentlyPlaying[value.regionInfo.regionName]?.[SMILDynamicEnum.dynamicValue]
			) {
				return true;
			}
		}
