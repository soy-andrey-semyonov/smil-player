		...
		this.sos.sync.onValue(async (key, value, groupName) => {
			if (groupName === this.groupName) {
				...
				if (key === 'sync-coordination' && value) {
					...
					debug(
						'[syncGroup] received %s: group=%s, type=%s, action=%s, syncIndex=%d, region=%s',
						messageCategory,
						this.groupName,
						value.type,
						action,
					...
					debug(
						'[syncGroup] stored sync-coordination: group=%s, key=%s, type=%s, syncIndex=%d, region=%s',
						this.groupName,
						coordKey,
						value.type,
						value.syncIndex,
				...
				if (key === 'elementState' && value) {
					...
					if (existing &&
						...
						debug(
							'[syncGroup] duplicate elementState, skipping: group=%s, state=%s, syncIndex=%d, region=%s',
							this.groupName,
							value.state,
							value.syncIndex,
							value.regionName,
					...
					debug(
						'[syncGroup] stored elementState: group=%s, key=%s, state=%s, syncIndex=%d, region=%s',
						this.groupName,
						stateKey,
						value.state,
						value.syncIndex,
